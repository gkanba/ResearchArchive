...
namespace settings{
    struct JsonConfig{
        ...
        std::vector<double_t> InitCenters;      // Every Initial Centers    (Size = VortexCout * 2) : 
        ...
        std::vector<double_t> Range_x;          // X Range of Calculation Domain                    :
        std::vector<double_t> Range_y;          // Y Range of Calculation Domain                    :
        int32_t Div_x;                          // X Division Count of Calculation Domain           :
        int32_t Div_y;                          // Y Division Count of Calculation Domain           :
...
int32_t main(int32_t argc, char** argv){
    ...
    // SuccessRadius Matrix Initialization
    Eigen::MatrixXd success_radius = Eigen::MatrixXd::Zero(div_mu + 1, div_gs + 1);

    // SuccessCount Matrix Initialization
    Eigen::MatrixXd success_counts = Eigen::MatrixXd::Zero(div_mu + 1, div_gs + 1);
    ...
    for(int32_t imu = 0; imu < div_mu + 1; imu++){
        for(int32_t igs = 0; igs < div_gs + 1; igs++){
            ...
            double_t smallest_radius = 10000000.0;
            ...
            for(int32_t ix = 0; ix < div_x + 1; ix++){
                ...
                for(int32_t iy = 0; iy < div_y + 1; iy++){
                    ...
                    // Update Only Small Vortex Location
                    ...
                    double_t error_sum = 0.0;
                    for(int32_t s = max_step - tau_step + 1; s <= max_step; s++){
                        error_sum += (centers_T.col(s) - l_centers_A.col(s)).norm();
                    }
                    double_t error_tmp = error_sum / tau_step;
                    double_t radius_tmp = std::sqrt(std::pow(pos_x(ix) - 1.0, 2) + std::pow(pos_y(iy), 2));

                    #pragma omp critical
                    {
                        // Row and Col Carefull!!!!!
                        error_mean(iy, ix) = error_tmp;
                        if(error_tmp < eta){
                            success_count++;
                        }
                        else{  
                            if(radius_tmp < smallest_radius && radius_tmp > 1.0e-8){
                                smallest_radius = radius_tmp;
                            }
                        }
                    }
                }
            ...
            // Store Data Matrix
            success_radius(imu, igs) = smallest_radius;
            success_counts(imu, igs) = success_count;

            // Store ErrorMean Matrix
            std::string imu_str = std::to_string(imu);
            std::string igs_str = std::to_string(igs);
            SaveMatrixObj("error_mean", config.RootDir + "/ErrorMean" + "-" + "mu" + imu_str + "-gs" + igs_str + ".csv", error_mean, IOFMT);
        }
    }
    ...
    SaveMatrixObj("success_radius", config.RootDir + "/ExpRadius.csv", success_radius, IOFMT);
    SaveMatrixObj("success_counts", config.RootDir + "/ExpCounts.csv", success_counts, IOFMT);
